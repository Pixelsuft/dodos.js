...
V86Starter.prototype.continue_init = async function(emulator, options) {
    ...
    function add_file(name, file) {
        ...
        file = {
            ...
            size2: file["size2"],
            step: file["step"],
            on_file_load: file["on_load"],
            zip_url: file["zip_url"],
        ...
        if (file.buffer instanceof ArrayBuffer) {
            ...
            files_to_load.push({
                ...
                url1: file.url1,
                url2: file.url2,
                on_file_load: file.on_file_load,
                size1: file.size1,
                size2: file.size2,
                zip_url: file.zip_url,
            });
    ...
    var cont = function(index) {
        ...
        if (f.loadable) {
            ...
            function result1(part1_content) {
                ...
                if (f.url2) {
                    v86util.load_file(f.url2, {
                        done: result2.bind(this),
                        progress: function progress(e) {
                            starter.emulator_bus.send("download-progress", {
                                file_index: index,
                                file_count: total,
                                ...
                                lengthComputable: e.lengthComputable,
                                total: e.total || f.size2,
                                loaded: e.loaded,
                            });
                        },
                    });
                } else {
                    result2.bind(this)(false);
                }
            }
            v86util.load_file(f.url1, {
                done: result1.bind(this),
                progress: function progress(e) {
